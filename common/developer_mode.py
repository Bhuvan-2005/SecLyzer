...
class DeveloperMode:
    ...
    def is_active(self):
        ...
        if self.active and self.activation_time:
            auto_disable_hours = self.config.get("security", {}).get(
    ...
    def _log_event(self, event_type, details):
        ...
        audit_file = self.config.get("security", {}).get(
            "audit_file", "/var/log/seclyzer/dev_mode.log"
        )
    ...
    def _show_warning(self):
        ...
        message = self.config.get("security", {}).get(
            "warning_message", "⚠ DEVELOPER MODE ACTIVE"
        )
