...
class Config:
    ...
    def _get_defaults(self) -> Dict:
        ...
        return {
            ...
            "databases": {
                ...
                "sqlite": {"path": "/var/lib/seclyzer/databases/seclyzer.db"},
    ...
    def get_redis_config(self) -> Dict:
        ...
        return {
            "host": os.getenv(
                "REDIS_HOST", self.get("databases.redis.host", "localhost")
            ),
            "port": int(
                os.getenv("REDIS_PORT", self.get("databases.redis.port", 6379))
            ),
    ...
    def get_influx_config(self) -> Dict:
        ...
        return {
            "url": os.getenv(
                "INFLUX_URL",
                self.get("databases.influxdb.url", "http://localhost:8086"),
            ),
            "token": os.getenv("INFLUX_TOKEN", ""),
            "org": os.getenv(
                "INFLUX_ORG", self.get("databases.influxdb.org", "seclyzer")
            ),
            "bucket": os.getenv(
                "INFLUX_BUCKET", self.get("databases.influxdb.bucket", "seclyzer")
            ),
        }
