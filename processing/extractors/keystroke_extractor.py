...
class KeystrokeExtractor:
        ...
        for message in self.pubsub.listen():
            if message['type'] == 'message':
                try:
                    ...
                    logger.error("Error processing keystroke event", error=str(e), event_type="keystroke")
    ...
    def _calculate_rhythm(self, df: pl.DataFrame) -> Dict:
        ...
        intervals = []
        for i in range(len(press_list) - 1):
            interval = (press_list[i + 1]['timestamp'] - press_list[i]['timestamp']) * 1000
            if 0 < interval < 5000:
        ...
        return {
            ...
            'typing_speed_wpm': 60000 / max(np.mean(intervals), 1) / 5,  # Rough WPM estimate
    ...
    def _save_features(self, features: Dict):
        ...
        try:
            ...
            print(f"[Keystroke Extractor] Saved & Published features | Dev mode: {features.get('dev_mode', False)}")
        except Exception as e:
