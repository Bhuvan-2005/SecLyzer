            ...
            if message["type"] == "message":
                try:
                    ...
                    if event_type == "keystroke":
                        print(
                            f"[{format_timestamp(timestamp)}] KEYSTROKE: {event['key']} ({event['event']})"
                        )
                        ...
                        if event["event"] == "move":
                            ...
                            if stats["mouse"] % 10 == 0:
                                print(
                                    f"[{format_timestamp(timestamp)}] MOUSE MOVE: x={event.get('x', 0):.0f}, y={event.get('y', 0):.0f}"
                                )
                        elif event["event"] in ["press", "release"]:
                            print(
                                f"[{format_timestamp(timestamp)}] MOUSE {event['event'].upper()}: {event.get('button', 'unknown')}"
                            )
                        elif event["event"] == "scroll":
                            print(
                                f"[{format_timestamp(timestamp)}] MOUSE SCROLL: delta={event.get('scroll_delta', 0)}"
                            )

                    elif event_type == "app":
                        print(
                            f"[{format_timestamp(timestamp)}] APP SWITCH: {event['app_name']} ({event['window_class']})"
                        )
